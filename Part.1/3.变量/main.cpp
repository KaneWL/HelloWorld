...
int main() {
	...
	int i = 0;
	//需要注意的是 如果你声明变量后不给它赋初始值 那么它的缺省(默认)值就会变成程序运行时的垃圾数据 所以一定一定一定在使用前确保你的变量已经初始化赋值过了

	//变量的命名规范请看: https://zh-google-styleguide.readthedocs.io/en/latest/google-cpp-styleguide/naming/#variable-names 7.4.变量命名
